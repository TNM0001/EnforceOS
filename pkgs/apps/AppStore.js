export default {
  name: "App Store",
  description: "EnforceOS App Store",
  ver: 1,
  type: "process",
  privileges: [
    {
  ...
  exec: async function (Root) {
    ...
    try {
      ...
      let host = "https://tnm0001.github.io/Enforce-AppStore/";
      ...
      if (appStoreModule.init) {
        ...
        async function installApp(pkg, app, force = false) {
          await fetch(`${host}pkgs/${pkg}/${app.assets.path}`)
            .then(async (e) => {
              console.log(await vfs.whatIs(`Root/Enforce/apps/${app.name}.app`));
              if (
                (await vfs.whatIs(`Root/Enforce/apps/${app.name}.app`)) ===
                  null ||
                force == true
              ) {
                let result = await e.text();

                await vfs.writeFile(`Root/Enforce/apps/${app.name}.app`, result);

                pages.appPage(app, pkg);
              } else if (
                (await vfs.whatIs(`Root/Enforce/apps/${app.name}.app`)) === "file"
              ) {
                await Root.Core.startPkg(
                  "data:text/javascript;base64," +
                    btoa(await vfs.readFile(`Root/Enforce/apps/${app.name}.app`)),
                  false,
                  true
                );
              }
          ...
          async appPage(app, pkg) {
            ...
            async function makeInstallOrOpenButton() {
              let localHash = new Hashes.MD5().hex(
                await vfs.readFile(`Root/Enforce/apps/${app.name}.app`)
              );
              ...
              const whatIsApp = await vfs.whatIs(
                `Root/Enforce/apps/${app.name}.app`
              );
                    ...
                    if (localHash !== appHash && whatIsApp !== null) {
                      ...
                      if (appCompatible === "ok") {
                        ...
                        const result = await Root.Modal.prompt(
                          "Notice",
                          `This app (made for ${app.compatibleWith}) may be incompatible with your current version of EnforceOS (${sysInfo.version}).\nAre you sure you want to continue installing?`,
                          container.elm
                        );
                ...
                whatIsApp != null
                  ? new Html("button")
                      ...
                      .on("click", async (e) => {
                        ...
                        if (result === true) {
                          await vfs.delete(`Root/Enforce/apps/${app.name}.app`);
